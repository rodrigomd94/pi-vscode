...
- The external-terminal VS Code widget now renders with pi theme tokens (`accent`, `muted`, `dim`, `warning`, `error`) instead of plain unstyled text; it emphasizes filenames with accent/bold styling, dims parent directories, shows a warning-colored dirty indicator, highlights multi-line selections via line-range color, and truncates long paths with middle truncation while preserving the filename suffix
